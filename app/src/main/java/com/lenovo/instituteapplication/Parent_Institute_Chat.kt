...
class Parent_Institute_Chat : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_parent_institute_chat)

        val songList = findViewById<RecyclerView>(R.id.pi_chat_Rv)
        val songObjects:MutableList<TeacherChatItem> = mutableListOf()
        val repository = Repository()
        val viewModelFactory = MainViewModelFactory(repository)
        val viewModel = ViewModelProvider(this, viewModelFactory).get(MainViewModel::class.java)
        viewModel.getAllPIChatMsg(institute_loggedin.institute_codes)
        viewModel.PIc_MsgResponse.observe(this, Observer { response->
